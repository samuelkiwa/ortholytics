...
#### avilella_at_gmail_dot_com
# This is a modified version of the runTwinscan.pl script which
# includes a flag in runTwinscan.pl so that when
# used, only the repeatmasker call will be done.

# I find this personally useful for creating the masked files and have
# them ready for the normal twinscan run in another computer. This can
# typically happen when having to switch between computers with low and
# high RAM, or when running twinscan multiple times on the same fasta
# files.
#### avilella_at_gmail_dot_com
...
my ($opt_d, $opt_blat, $opt_B, $opt_RM, $opt_r, $opt_x, $opt_v,$onlyrepeatmasker);
GetOptions(
	   "d=s" => \$opt_d,
           "B=s" => \$opt_B,
	   "RM=s" => \$opt_RM,
	   ...
	   "x" => \$opt_x,
	   "v" => \$opt_v,
	   'onlyrepeatmasker' => \$onlyrepeatmasker,
          );
my $usage = "
